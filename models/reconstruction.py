...
def CommonRegion(walls, pwalls, width, ratio=0.25, band=10, threshold=10):
    ...
    #中心のx座標に基づいてソート→隣接しているであろう順番に並ぶ
    xyxy = walls[:, :4]
    ...
    for i in range(num-1):
        ...
        w0 = p0[2] - p0[0]
        w1 = p1[2] - p1[0]
        if p0[2] < p1[0]:  #二つの壁が重なっていない場合 two boxes have no intersection
            dis = p1[0] - p0[2]
            n0 = param[i][:3]
            n1 = param[i+1][:3]
            s = 1 + np.sum(n0 * n1)
            if dis > threshold and s < 0.003:  #距離が閾値を超え、平行でない場合 the distance is larger than threshold and is almost parallel (180'), their intersection is infinite.
                #共通領域を無限とする
        ...
        else:  # 二つの壁が重なっている場合　two boxes have intersection
...
def PreProcess(planes, params, lines, threshold=(0.5, 0.1, 0.1, 0.5)):
    ...
    #床と壁が両方検出されたけど壁がない場合はスコアが高いほうだけにする
...
def OptimizerLayout(pwalls, lines, common, K, size, downsample=8, opt=True):
    ...
    if num > 1:
        for i in range(num-1):
            ...
            minv = np.min(offset, axis=0)
            """
            inside 負の場合 : lの両サイドに境界点が存在する
                   正の場合 : lの片側にしか境界定位が存在しない
            """
            inside = maxv * minv 
            # whether common region exists detection lines
            # 共通領域内に検出線があるか
            bound = np.reshape(bound, (4, 1, 2)) #ブロードキャスト演算を可能に
            offset = bound[:, :, 0] - lines[:, 0] * \
                bound[:, :, 1] - lines[:, 1]  # 4 * N
            maxv = np.max(offset, axis=0)
            minv = np.min(offset, axis=0)
            """
            boundポイントの集合を横切る線をlinesから検出
            横切るとは、boundのポイントが両サイドにある状態 (maxv * minv < 0)
            """
            ...
            if inside < 0:
                ...
                if len(inlines) > 0:
                    ...
                    # 検出線が存在し、平面が交差する場合、
                    # 最もスコアが高い検出船をdtl1に格納し、caseリストに番号"0"を追加
...
def CalculateIntersectionPoint(p0, p1, p2, K, UD=0, downsample=2, size=(1024, 1280)):
    ...
    A = coefficient[:, :3] #係数行列
    B = -1 * coefficient[:, 3] #定数ベクトル

    res = np.linalg.solve(A, B) #解
    # project 3d to 2d
...
def GenerateLayout(pwalls, case, dtls, pfloor, pceiling, K, size, downsample=2, upsample=8):
    ...
    for i in range(num-1):
        p0 = pwalls[i]
        p1 = pwalls[i+1]
        if case[i] == 0 or case[i] == 1:  # 2つの壁が交わる two walls intersect
            #天井との交点
            ...
            fake_plane = CalculateFakePlane(fake_line, K_inv) #見えていない壁 
            point0 = CalculateIntersectionPoint(
            ...
            param_layout.append(p0)
            # print("point0", point0)
            param_layout.append(None) #疑問：Noneを入れる意味は？
            # print("add None" )
            pass
        else:  # two occlusion line exist, the case maybe solve by add an infinite plane, TODO
            # 2つのocclusion lineが存在する場合：無限平面を追加しているが、未完成
            dtl = dtls[i]
            dtl0 = [dtl[0], dtl[1], 1]
            dtl1 = [dtl[0], dtl[2], 1]
            fake_line0 = np.array(
...
def ConvertLayout(img, ups, downs, attribution, pwalls=None, pfloor=None, pceiling=None, K=None, ixy1map=None, oxy1map=None, valid=None, pixelwise=None):
    ...
    #print("w, h", w,h)
    #print("at", np.ones([h, w]))
    colors = np.random.uniform(10, 250, (30, 3))
    floor = (0, 255, 0)
    ceiling = (255, 0, 0)
    segmentation = 2 * np.ones([h, w])  # [0: ceiling, 1: floor, 2...:walls]
    valid_pwalls = []
...
def Reconstruction(planes, params_ins, lines, K, size, threshold=(0.2, 0.05, 0.05, 0.5), downsample=8, cat='both'):
    ...
    # print("reconstruction_lines", lines)
    # print("pwall", pwalls)
    # no optimizer
