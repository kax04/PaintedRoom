...
def post_process(outputs, K1=20, K2=200, Mnms=1):
    ...
    #print("line_region_af_nms", line_region)
    ...
    #print("xs", xs)
    #print("reg_offset", reg_offset)
    #print("ys", ys)
    #draw_line(xs, ys, reg_alpha,reg_offset,  width, height)
    ...
    lines=line_array(xs, ys, reg_alpha)
    #draw_line_on_white_image(width, height, lines)

    #リスト内にtensorがある場合にリスト全体をtensorにする
    dt_lines = torch.stack([xs, ys, reg_alpha, scores], dim=2).data.cpu().numpy()  #B*K*4

    #print("dt_lines", dt_lines)
    
    #draw_line_on_white_image(width, height, dt_lines[:][0:3])
    ...
    return dt_planes, dt_lines, plane_params_instance, plane_params_pixelwise, height

def draw_line(xs, ys, thetas, offsets,  width, height):
    image = np.ones((height, width, 3), np.uint8) * 255
    # θのcosとsinを計算

    # 傾きと切片を計算
    # 線の方程式は xcosθ + ysinθ = 0 で表される
    # y = -(cosθ/sinθ)x

    for i in range(len(xs[0])):
        x=xs[0][i]
        y=ys[0][i]
        cos_theta = math.cos(thetas[0][i])
        sin_theta = math.sin(thetas[0][i])
        offset=offsets[0][i]

        intercept = (x+offset)*cos_theta + y*sin_theta

        if sin_theta != 0:
            slope = -cos_theta / sin_theta

            # y = mx + b の形式で線を描くための2点を計算
...
def line_array(xs, ys, reg, length=30):
    lines=[]
    xs = xs.data.cpu().numpy()
    ys = ys.data.cpu().numpy()
    reg = reg.data.cpu().numpy()
    #print("line_array_xs", xs)

    for i in range(len(xs[0])):
        #print("i :", i)
        x_start=xs[0][i]
        y_start=ys[0][i]
        angle_reg=np.deg2rad(reg[0][i])
        x_end = x_start + length*np.cos(angle_reg)
        y_end = y_start + length*np.sin(angle_reg)
        lines.append([int(x_start), int(y_start), int(x_end), int(y_end)])
 
    return lines

def draw_line_on_white_image(width, height, lines, color=(0, 0, 0), thickness=2):
    print("white image : ", width, "×", height)
    """
    指定したサイズの白い画像に線を描画する関数
    
    Args:
    width (int): 画像の幅
    height (int): 画像の高さ
    start_point (tuple): 線の始点 (x, y)
    end_point (tuple): 線の終点 (x, y)
    color (tuple): 線の色 (B, G, R)
    thickness (int): 線の太さ

    Returns:
    numpy.ndarray: 線が描画された画像
    """
    # 白い画像を作成
    image = np.ones((height, width, 3), np.uint8) * 255
    #print(lines)
    ...
    for line in lines:
        ...
        cv2.circle(image, start_point,1, color, thickness)
    cv2.imshow('Image', image)
    cv2.waitKey(0)
...
def DisplayLayout(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, re_seg, re_depth, re_polys,_re_seg,  line_img, segs_gt, label, iters):
    palette = [
    ...
    # 6色 + ランダム20色 + 新たなランダム10色
    palette = np.concatenate([
        np.array(palette)[[0, 1, 5, 10, 16, 19]],
        np.random.uniform(0, 1, (20, 3)),  # 既存のランダム20色
        np.random.uniform(0, 1, (50, 3))   # 新たに追加する10色
    ], axis=0)
    ...
    for i in np.unique(re_seg):
        if i == -1:
            continue
        else:
            _segs_re.append(re_seg==i)
    ...
    _segs_re = np.array(_segs_re).astype(np.int)
    cost1 = ((_segs_gt[:, np.newaxis] + _segs_noopt)==2).sum((2, 3))
    cost2 = ((_segs_gt[:, np.newaxis] + _segs_opt)==2).sum((2, 3))
    cost3 = ((_segs_gt[:, np.newaxis] + _segs_re)==2).sum((2, 3))
    r1, c1 = linear_sum_assignment(-1*cost1)
    r2, c2 = linear_sum_assignment(-1*cost2)
    r3, c3 = linear_sum_assignment(-1*cost3)

    #print("_seg_gt", _segs_gt)
    #np.savetxt('_seg_gt.txt', _segs_gt)

    color1 = np.arange(2, len(_segs_gt)+2)
    #print("vusual_seg", color1)
    ...
    for i in range(len(_segs_re)):
        if i in c3:
            idx = np.where(c3 == i)[0][0]
            color4.append(color1[r3[idx]])
        else:
            color4.append(None)
    j = 0
    for i in range(len(color4)):
        if color4[i] is None:
            color4[i] = 2+len(color1) + len(color2)+ len(color3) + j
            j = j + 1

    masks_re = np.zeros_like(img, dtype=np.float32)  # re
    bound_re = np.zeros_like(img[:,:,0], dtype=np.float32)    
    ...
    img7=img
    for i in range(len(color4)):
        alpha_fill = (_segs_re[i] == 1)[..., None].astype(np.float32)
        sx = cv2.Sobel(alpha_fill, cv2.CV_32F, 1, 0, ksize=5)
        sy = cv2.Sobel(alpha_fill, cv2.CV_32F, 0, 1, ksize=5)
        alpha_edge = (sx ** 2 + sy ** 2) ** 0.5
        alpha_edge /= max(0.001, np.max(alpha_edge))
        alpha_edge = alpha_edge[..., None]
        alpha_fill *= 0.5
        color = palette[color4][i]
        img7 = img7 * (1 - alpha_fill) + alpha_fill * color
        img7 = img7 * (1 - alpha_edge) + alpha_edge * color

        bound_re[alpha_edge[:,:,0]>0]=1.
    ...
    #cv2.imwrite(f'result/8{iters}_check.png', img3 * 255)
    # row1 = np.concatenate([img[:360], img1[:360], line_img[:360]*(1/255)], axis=1)
    # row2 = np.concatenate([img2[:360], img3[:360], img7[:360]], axis=1)
    # final_image = np.concatenate([row1, row2], axis=0)
    # cv2.imwrite(f'debug/9{iters}_check.png', final_image * 255)
    return img7

def _DisplayLayout(img, segs_opt, depth_opt, polys_opt, segs_noopt, depth_noopt, polys_noopt, segs_gt, label, iters):
    palette = [
        (0.12156862745098039, 0.4666666666666667, 0.7058823529411765),
        (0.6823529411764706, 0.7803921568627451, 0.9098039215686274),
        (1.0, 0.4980392156862745, 0.054901960784313725),
...
def MakeLayoutImage(img, segs_gt, label, iters):
    palette = [
        (0.12156862745098039, 0.4666666666666667, 0.7058823529411765),
        (0.6823529411764706, 0.7803921568627451, 0.9098039215686274),
        (1.0, 0.4980392156862745, 0.054901960784313725),
    ...
    for i in range(len(color1)):
        alpha_fill = (_segs_gt[i]==1)[..., None].astype(np.float32)
        # alpha_fill を img1 の形状に合わせてリサイズ
        #alpha_fill_resized = cv2.resize(alpha_fill, (img1.shape[1], img1.shape[0]), interpolation=cv2.INTER_NEAREST)
        # もし必要なら、チャネル次元も追加します
        #alpha_fill = np.expand_dims(alpha_fill_resized, axis=-1)  
        sx = cv2.Sobel(alpha_fill, cv2.CV_32F, 1, 0, ksize=5)
        sy = cv2.Sobel(alpha_fill, cv2.CV_32F, 0, 1, ksize=5)
        alpha_edge = (sx ** 2 + sy ** 2) ** 0.5
        alpha_edge /= max(0.001, np.max(alpha_edge))
